 ...
 */
var _argon2: IArgon2 = require("argon2");
...
export class Kdf {
    ...
    private static _defaultOptions: IArgon2Options = { argon2d: false, memoryCost: 12, parallelism: 2, timeCost: 10 };
    /**
     * generate a combined "version+hash+salt" output 60 characters long.
     */
    public static hashSync(inputSecret: string, options = Kdf._defaultOptions): string {

        var salt = _argon2.generateSaltSync();
        return _argon2.encryptSync(inputSecret, salt, options);
    }
    public static hashAsync(inputSecret: string, options = Kdf._defaultOptions): Promise<string> {
    ...
    public static verifySync(inputSecret: string, kdfOutputHash: string): boolean {
        return _argon2.verifySync(kdfOutputHash, inputSecret);
    }

    public static verifyAsync(inputSecret: string, kdfOutputHash: string): Promise<void> {
 ...
 */
export var passwordStrengthEstimator: {
    (password: string,
